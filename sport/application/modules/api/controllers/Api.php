...
public function time_today_test()
    {   
	...
	public function joinevent_user()
	{
		...
		if(!empty($_REQUEST['event_id']) && !empty($_REQUEST['user_id']))
        {
			...
			$where="(user_id	='" .  $event_user_id . "' AND join_id ='" . $user_id . "' AND event_id ='" . $event_id . "') AND (status = 1 OR status = 0)";
		...
		{	
			...
			// notification start
				...
				$insert_notification = $this->common->insertData('notification_tbl',array('user_id' => $event_user_id,'message' => "Wants to Join Your Event",'date'=>$today,'user_send_from'=>$user_id,'type'=>"Join",'event_id'=>$event_id,'unique_id'=>$insert_id));
								
				$notification = array('user_id' => $event_user_id,'message' => "Wants to Join Your Event",'date'=>$today,'user_send_from'=>$user_id,'type'=>"Join",'event_id'=>$event_id,'unique_id'=>$insert_id);
				
				$user_data_from = $this->common->getData('user',array('id'=>$user_id),array('single'));

				$from_name_notification = $user_data_from['name'];

				$sendmsg =  array('title'=>$from_name_notification,'body'=>"Wants to Join Your Event");
	 ...
	 // For IOS notification
      function push_iOS($token, $msg, $alert) {
        ...
        if (!empty($this->pem_Pro) && !empty($this->passPhrase)) {
            ...
            // stream_context_set_option($tContext, 'ssl', 'local_cert', 'pushdevcert.pem');
	...
	public function notification_list()
	{
		if(!empty($_REQUEST['user_id']))
        {
        	...
        	foreach ($user_info as $key => $value)
        	{
        		...
        		$blockinfo = $this->common->getData('user_block',array('user_id'=>$user_id,'block_uid'=>$value['id']),array('single'));
				if($blockinfo)
				{
					$block_status = 1;
				}
				...
				$arr[]=array('id'=>$value['id'],'name'=>$value['name'],'email'=>$value['email'],'image'=>$image,'message'=>$value['message'],'date'=>$value['date'],'type'=>$value['type'],'event_id'=>$value['event_id'],'Notification_id'=>$value['Notification_id'],'unique_id'=>$value['unique_id'],'block_status'=>$block_status);
            ...
            {
                $this->response(true,"No Record Found",array("notification_info" =>array()));
            }
        }
        else
        {
	...
	public function event_detail()
	{
		if(!empty($_REQUEST['id']))
		{
			...
			$user_event_info_main = $this->common->getData('sport_event',array('id' => $id),array('single'));

			if(!empty($user_event_info_main['user_id']))
			{
				$event_user_main_id = $user_event_info_main['user_id'];
			}
			else
			{
				$event_user_main_id = 0;
			}
			...
			foreach ($result as $key => $value) {
				...
				if(!empty($value['join_user']))
				{
						...
						foreach($result_user as $user_data)
						{
							...
							if($user_data['id'] == $event_user_main_id)
							{
								$owner_status = 1;
							}
							else
							...
							$user_info[]=array('id'=>$user_data['id'],'image'=>$image,'owner_status'=>$owner_status);
						}
...
;
	...
	public function my_sport()
	{
		if(!empty($_REQUEST['user_id']))
		{
			...
			$total_join_user =	count($join_user_arr);
        	...
        	if (in_array($user_id,$join_user_arr))
        	{
        		...
        		$arr[]=array('id'=>$value['id'],'title'=>$value['title'],'game_id'=>$value['game_id'],'event_user_type'=>$value['event_user_type'],'event_time'=>$value['event_time'],'event_duration'=>$value['event_duration'],'event_participant_no'=>$value['event_participant_no'],'event_description'=>$value['event_description'],'status'=>$value['status'],'game_name'=>$value['game_name'],'game_image'=>$game_image,'join_user'=>$value['join_user'],'Sport_status'=>$Sport_status,'event_address'=>$value['event_address'],'latitude'=>$value['latitude'],'longitude'=>$value['longitude'],'total_join_user'=>$total_join_user);
        	}
	...
	{
		if(!empty($_REQUEST['user_id']) && !empty($_REQUEST['event_id']) && !empty($_REQUEST['invite_id']))
		{
			$user_id = $_REQUEST['user_id'];
			$event_id = $_REQUEST['event_id'];
			...
			{
				$where_event="id='". $event_id."'";
				$value_event = $this->common->getData('sport_event',$where_event,array('single'));
				$send_from = $invite_id;



				// notification start
				$user_data = $this->common->getData('user',array('id'=>$user),array('single'));
				$ios_token = $user_data['ios_token'];
				$android_token = $user_data['android_token'];
				$today = Date('Y-m-d H:i:s'); 
				$insert_notification = $this->common->insertData('notification_tbl',array('user_id' => $user,'message' =>"Has to you Invited Event",'date'=>$today,'user_send_from'=>$send_from,'type'=>"Invite",'event_id'=>$_REQUEST['event_id']));
				
				$notification = array('user_id' => $user,'message' =>"Has to you Invited Event",'date'=>$today,'user_send_from'=>$send_from,'type'=>"Invite",'event_id'=>$_REQUEST['event_id']);

				$user_data_from = $this->common->getData('user',array('id'=>$invite_id),array('single'));

				$from_name_notification = $user_data_from['name'];

				$sendmsg =  array('title'=>$from_name_notification,'body'=>"Has to you Invited Event");
				...
				// notification end
					...
					$this->response(true,"Invite friend Successfully");
		}
		else
		{
			$this->response(false,"Missing Parameter.");
		...
		if(!empty($_REQUEST['user_id']) && !empty($_REQUEST['follow_uid']))
		{
			 ...
			 $follow_uid = $_REQUEST['follow_uid'];
				...
				if ($status == 1) {
					...
					if(empty($value))
						{	
							$insert = $this->common->insertData('user_following',array('user_id' => $user_id,'following_id' => $follow_uid,'date_follow' => Date('Y-m-d H:i:s')));
							 ...
							 $user_data = $this->common->getData('user',array('id'=>$follow_uid),array('single'));
							 	...
							 	$android_token = $user_data['android_token'];
							 	$today = Date('Y-m-d H:i:s'); 
								$insert_notification = $this->common->insertData('notification_tbl',array('user_id' => $follow_uid,'message' => "Started Following You",'user_send_from'=>$user_id,'date'=>$today,'type'=>"Following"));
							
							
							$notification = array('user_id' => $follow_uid,'message' => "Started Following You",'user_send_from'=>$user_id,'date'=>$today,'type'=>"Following");
	...
	{
		if(!empty($_REQUEST['join_id']) && !empty($_REQUEST['event_id']) &&!empty($_REQUEST['status']) &&!empty($_REQUEST['notification_id']) &&!empty($_REQUEST['unique_id']))
		{
			...
			if($status == 1)
			{
				...
				$android_token = $user_data['android_token'];
				$today = Date('Y-m-d H:i:s'); 
				$insert_notification = $this->common->insertData('notification_tbl',array('user_id' => $join_id,'message' => "Now Joinee of Event",'date'=>$today,'user_send_from'=>$user_id,'type'=>"Event",'event_id'=>$_REQUEST['event_id']));
								
				$notification = array('user_id' => $join_id,'message' => "Now Joinee of Event",'date'=>$today,'user_send_from'=>$user_id,'type'=>"Event",'event_id'=>$_REQUEST['event_id']);
				...
				$sendmsg =  array('title'=>$from_name_notification,'body'=>"Now Joinee of Event");
			...
			else
			{
				...
				if($result){
					...
					$sendmsg =  array('title'=>$from_name_notification,'body'=>"Cancel Join Request");
	...
	function compareByName($a, $b) {
  		return strcmp(strtolower($a["game_name"]),strtolower($b["game_name"]));
	}
	...
	public function game_list()
	{
        ...
        if(!empty($result)){
        	 ...
        	 foreach ($result as $key => $value) {
        	  ...
        	  usort($arr, array($this,'compareByName'));
	...
	public function user_list()
	{
		...
		{
			$follow_status = 1;
			$date_follow = $follow['date_follow'];
			$time_elapsed = $this->timeAgo($follow['date_follow']);
		}
		else
		{
			$follow_status = 2;
		...
		$arr[]=array('id'=>$value['id'],'name'=>$value['name'],'email'=>$value['email'],'image'=>$image,'follow_status'=>$follow_status,'block_status'=>$block_status,'created_at'=>$created_at,'date_follow'=>$date_follow,'time_elapsed'=>$time_elapsed);
		}
	...
	{		
		...
		if($user){
			...
			$update = $this->common->updateData('user',array('ios_token' =>$_POST['ios_token'], 'android_token' => $_POST['android_token']),array('id' => $user['id']));
			if($update){				
	...
	public function getProfile()
	{
		...
		if(!empty($value))
		{
			...
			$blockinfo = $this->common->getData('user_block',array('user_id'=>$user_id,'block_uid'=>$_POST['id']),array('single'));
		if($blockinfo)
		{
			$block_status = 1;
		}
		else
		{
			...
			$age= date_diff(date_create($value['user_dob']), date_create('today'))->y;
		$user=array('id'=>$value['id'],'name'=>$value['name'],'email'=>$value['email'],'image'=>$value['image'],'social'=>$value['social'],'ios_token'=>$value['ios_token'],'android_token'=>$value['android_token'],'created_at'=>$value['created_at'],'user_address'=>$value['user_address'],'Age'=>$age,'user_dob'=>$value['user_dob'],'status'=>$value['status'],'user_info'=>$value['user_info'],'image'=>$image,'game_logo'=>$game_image,'block_status'=>$block_status);
...
// return mysqli_connect('localhost', $db['username'], $db['password'], $db['database']);}
	...
	public function chat()
	{
		...
		if(!empty($_REQUEST['user_from']))
		{
			...
			if($result)
			{
				...
				$from_name_not = $user_data_from['name'];
			

				if(!empty($user_data_from['image']))
                 	{
                 		$from_image_not = base_url('/assets/userfile/profile/'.$user_data_from['image']);
                 	}
                 	else
                 	{
                 		$from_image_not = '';
				...
				$notification = array('user_id' => $user_to,'message' => $_REQUEST['message'],'user_send_from'=>$_REQUEST['user_from'],'date'=>$today,'type'=>"Message",'from_name'=>$from_name_not,'from_name'=>$from_name_not,'from_image'=>$from_image_not);
				
				$sendmsg =  array('title'=>$from_name_not,'body'=>$_REQUEST['message']);
	...
	{
		if(!empty($_REQUEST['get_type']) && !empty($_REQUEST['user_latitude']) && !empty($_REQUEST['user_longitude']) && !empty($_REQUEST['user_id']))
        {

        	$user_latitude = $_REQUEST['user_latitude'];
        	$user_longitude = $_REQUEST['user_longitude'];
        	$user_id_login = $_REQUEST['user_id'];
		...
		foreach ($result as $key => $value) {
			...
			$event_time_data=$value['event_time'];
        		$today_data = Date('Y-m-d H:i:s'); 
        		
        		if($event_time_data < $today_data)
        		{
        			 $sport_status = 2;
        		}
        		else
	...
	$where = "user_from = '".$_POST['id']."' or user_to = '".$_POST['id']."' or  `group_id` IN ('".$comma_separated."')";
		...
		if(!empty($main_data)){
			...
			foreach ($data as $key => $value) {
				...
				$blockinfo = $this->common->getData('user_block',array('user_id'=>$_POST['id'],'block_uid'=>$value['user_id']),array('single'));
				if($blockinfo)
				{
					$block_status = 1;
				}
				...
				$user[]= array('message'=>$value['message'],'created_at'=>$value['created_at'],'image' => $value['image'],'message_staus'=>$value['message_staus'],'id'=>$value['id'],'user_id'=>$value['user_id'],'user_name'=>$user_info['name'],'block_status'=>$block_status);
			}
