	...
	public function invite_friends()
	{
		if(!empty($_REQUEST['user_id']) && !empty($_REQUEST['event_id']))
		{
			...
			// notification start
					...
					$android_token = $user_data['android_token'];
					$today = Date('Y-m-d H:i:s'); 
					$insert_notification = $this->common->insertData('notification_tbl',array('user_id' => $user_id,'message' =>"Has to you Invited Event",'date'=>$today,'user_send_from'=>$send_from,'type'=>"Invite",'event_id'=>$_REQUEST['event_id']));



									
					$notification = array('user_id' => $user_id,'message' =>"Has to you Invited Event",'date'=>$today,'user_send_from'=>$send_from,'type'=>"Invite",'event_id'=>$_REQUEST['event_id']);
